#!/usr/bin/env python3
"""
Control: Auto Scaling group uses multiple Availability Zones.

describe_auto_scaling_groups returns the AvailabilityZones the ASG is
configured to span. An ASG spanning only one AZ has no resilience against
an AZ-level outage.
"""
...
CONTROL_NAME = "ASG Uses Multiple Availability Zones"
...
def evaluate_asg(asg):
    ...
    if len(azs) >= 2:
        return "COMPLIANT", f"Spans {len(azs)} Availability Zones: {', '.join(sorted(azs))}"

    return "NON_COMPLIANT", f"Spans only 1 Availability Zone: {azs[0]}"
...
def write_csv(results, account_id):
    ...
    filename = f"asg_multi_az_{account_id}_{timestamp}.csv"
